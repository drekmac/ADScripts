...
function Get-StaleComputers {
    ...
    $stale = Get-ADComputer -Filter {LastLogonTimeStamp -lt $Time -and (Operatingsystem -like 'Windows XP*' -or Operatingsystem -like 'Windows Vista*' -or Operatingsystem -like 'Windows 7*' -or Operatingsystem -like 'Windows 8*' -or Operatingsystem -like 'Windows 10*' -or Operatingsystem -like 'Windows 2000 Pro*') } -Properties $properties -SearchBase $searchbase | select-object Name,Enabled,$hash_LastLogonTimeStamp,PasswordLastSet,Modified,Created,Operatingsystem,Description,Location,IPv4Address,DistinguishedName,SID,LastLogonDate | Sort-Object -Property LastLogonDate
    if ($null -ne $file){
...
##########-do work-##########
$Stale = Get-StaleComputers -days $Disable -file $file -SearchBase $searchbase
$StaleDelete = $Stale | Where-Object LastLogonTimestamp -lt $DeleteDate
$Stalecount = $Stale.Count
$DeleteCount = $StaleDelete.Count
$DisableCount = $StaleCount - $DeleteCount
if(!$whatif){
    if (!$confirm) {
        Write-Host "WARNING: $DisableCount computers are about to be disabled and $DeleteCount computers are about to be deleted (if they are disabled)." -ForegroundColor Yellow -BackgroundColor Red
    ...
    "Whatif - $DisableCount would have been disabled and $deletecount would have been deleted."
}